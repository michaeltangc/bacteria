...
class ImageDisp:
	def __init__(self, master):
		...
		self.buttonList = [Radiobutton(self.radioBtns, text=text, variable=self.label_val, value=val, command=self._select_label,
				indicatoron=0, width=15, height=1) for i, (text, val) in enumerate(self.txt_label)]
	...
	def _display(self):
		...
		for i, b in enumerate(self.buttonList):
			b.grid(row=i, column=1, pady=(25,0)) if i == 4 else b.grid(row=i, column=1, pady=(10,0))
		# control buttons
